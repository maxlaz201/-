        ...
        try:
			...
			'🗿🗿🗿\n     🗿🗿🗿\n       🗿🗿🗿\n        🗿🗿🗿\n         🗿🗿🗿\n          🗿🗿🗿\n          🗿🗿🗿\n      🗿🗿🗿🗿\n 🗿🗿🗿🗿🗿🗿\n 🗿🗿🗿  🗿🗿🗿\n    🗿🗿       🗿🗿')
                    ...
                    if message.text.lower() == "чс":
                    	await message.edit(f"<a href=tg://user?id={user.id}>{user.first_name}</a> <b>был(-а) занесен(-а) в Черный Список.</b>")
                    	await message.client(functions.contacts.DeleteContactsRequest(id=[user.id]))
                    if message.text.lower() == "прибить":
                    	await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> прибил(-а) <a href=tg://user?id={user.id}>{user.first_name}</a>")
                    if message.text.lower() == "больно":
                    	await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> громко крикнул(-а) <a href=tg://user?id={user.id}>{user.first_name}</a>")
                    if message.text.lower() == "+":
                    	await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> плюсанул(-а) <a href=tg://user?id={user.id}>{user.first_name}</a>")
                    if message.text.lower() == "дать пизды":
                    	await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> дал(-а) пизды <a href=tg://user?id={user.id}>{user.first_name}</a>")
        except: pass
